...
public interface ArtworkRepository extends JpaRepository<Artwork,Long> {
    ...
    @Query("select new world.creve.platform.dto.ArtworkSummaryResponse(a.artworkId,a.slug,a.title,a.mainImageUrl,c.creatorId,c.slug,c.name) from Artwork a join Creator c on a.creatorId=c.creatorId where a.creatorId=:creatorId and a.status='PUBLISHED' and c.status='PUBLISHED' order by a.createdAt desc,a.artworkId desc") List<ArtworkSummaryResponse> findPublishedArtworksByCreatorId(@Param("creatorId") Long creatorId,Pageable pageable);
